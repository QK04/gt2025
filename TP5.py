...
def adjacency_matrix(vertices, edges):
    n = len(vertices)
    vertex_index = {vertex: idx for idx, vertex in enumerate(vertices)} # Map vertex names to indices
    adj_matrix = [[float('inf')] * n for _ in range(n)] # Initialize distances to infinity
    
    for i in range(n):
        adj_matrix[i][i] = 0  # Zero distance to self

    for (u, v), weight in edges.items():
        u_idx, v_idx = vertex_index[u], vertex_index[v] 
        adj_matrix[u_idx][v_idx] = weight # Directed graph
...
def dijkstra(adj_matrix, vertex_index, source, target):
    ...
    pq = [(0, source_idx)]  # (distance, vertex)
        ...
        for neighbor in range(n):
            ...
            if weight < float('inf'):
                ...
                if distance < dist[neighbor]:
                    ...
                    heapq.heappush(pq, (distance, neighbor)) # Add neighbor to priority queue
